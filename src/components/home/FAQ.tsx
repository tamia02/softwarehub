  ...
  return (
    <Section id="faq" className="py-14 md:py-20">
      <div className="mx-auto max-w-[720px] text-center lg:max-w-[880px]">
        <h2 className="t-h2 text-ink-line">Questions?</h2>
      </div>
        ...
        {items.map((item, i) => {
          ...
          return (
            <FadeUp key={item.q} index={Math.min(i, 5)} className={cn("px-5 md:px-7", i > 0 && "border-t-2 border-ink-line")}>
              <h3>
                <button onClick={() => setOpen(isOpen ? -1 : i)} aria-expanded={isOpen} aria-controls={`faq-panel-${i}`} id={`faq-btn-${i}`} className="flex w-full cursor-pointer items-center justify-between gap-4 py-5 text-left text-[17px] font-bold leading-[1.2] text-ink-line hover:text-accent-2 md:text-[18px]">
                  {item.q}
