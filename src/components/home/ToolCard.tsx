  ...
  return (
    <motion.article
      ...
      className={cn(
        ...
        proOnly && "border-amber-200/70 bg-[linear-gradient(180deg,#fffbf0,white_40%)]",
      )}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="mark-gray">
          <VendorMark name={tool.vendor} hue={tool.hue} />
        </div>
        {tool.badge && <Badge tone={badgeTone[tool.badge]}>{tool.badge}</Badge>}
      </div>

      <div className="flex-1">
        <h3 className="text-[17px] font-extrabold leading-snug">{tool.name}</h3>
        <p className="mt-1 text-sm font-semibold text-primary">{tool.offerTitle}</p>
        <p className="mt-2 text-sm text-ink-muted">{tool.blurb}</p>
      </div>

      <div className="flex items-center justify-between gap-2 border-t border-line/80 pt-4">
        <span className="rounded-full bg-bg-soft px-2.5 py-1 text-xs font-bold text-ink-muted" title="Listed annual retail value">
